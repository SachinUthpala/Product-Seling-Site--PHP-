      ...
      <section class="header-section ethanet-switches" style="height: 80vh;">
        ...
        <div class="products">
          ...
          <div class="product-items" >
            ...
            $sql_FE_NPE= "SELECT * FROM `ethanet_switches` WHERE poeType = 'FE_PE' and company = 'LEVEL_ONE'";
            $stmt_FE_NPE = $conn->prepare($sql_FE_NPE);
            $stmt_FE_NPE->execute();
          ...
          <div class="product-items" >
            ...
            $sql_gig_NPE_cisco= "SELECT * FROM `ethanet_switches` WHERE poeType = 'GIG_NPE' and company = 'CISCO'";
            $stmt_gig_NPE_cisco = $conn->prepare($sql_gig_NPE_cisco);
            $stmt_gig_NPE_cisco->execute();

            
            ?>
            
            <?php while($result_gig_NPE_cisco = $stmt_gig_NPE_cisco->fetch(PDO::FETCH_ASSOC)){ ?>
            <!-- ----------------------------- -->
            <div class="product-card">
              <div class='main'>
          ...
          <div class="product-items" >
            ...
            $sql_gig_PE_cisco= "SELECT * FROM `ethanet_switches` WHERE poeType = 'GIG_PE' and company = 'CISCO'";
            $stmt_gig_PE_cisco = $conn->prepare($sql_gig_PE_cisco);
            $stmt_gig_PE_cisco->execute();

            
            ?>
            
            <?php while($result_gig_PE_cisco = $stmt_gig_PE_cisco->fetch(PDO::FETCH_ASSOC)){ ?>
            <!-- ----------------------------- -->
            <div class="product-card">
              <div class='main'>
